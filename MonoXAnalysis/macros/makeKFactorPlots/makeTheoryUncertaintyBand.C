...
void makeTheoryUncertaintyBand(string inputFileName, string outputDIR , bool useNewTheoryUncertainties, string observable){
  ...
  if(not useNewTheoryUncertainties){
    ...
    zw_ren_unc->GetXaxis()->SetTitleOffset(1.1);
    zw_ren_unc->GetXaxis()->SetTitleSize(0.05);
    zw_ren_unc->GetYaxis()->SetTitle("Z/W Variation/Nominal");
    zw_ren_unc->GetYaxis()->SetTitleSize(0.045);
    zw_ren_unc->GetYaxis()->SetTitleOffset(1.15);

    zw_ren_unc->GetYaxis()->SetRangeUser(-0.25,0.25);
    ...
    leg.SetBorderSize(0);
    leg.SetNColumns(2);
    leg.AddEntry(zw_ren_unc,"QCD #mu_{r}","L");
    leg.AddEntry(zw_fac_unc,"QCD #mu_{f}","L");
