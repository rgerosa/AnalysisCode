...
void prepostTM(string fitFilename, string templateFileName, string observable) {
  ...
  TH1* dthist = (TH1*)dfile->Get(("datahisttopmu_"+observable).c_str());
  TH1* wlhist = (TH1*)pfile->Get("shapes_fit_b/ch7/ZJets_TM");
  TH1* tthist = (TH1*)pfile->Get("shapes_fit_b/ch7/WJets_TM");
  TH1* dihist = (TH1*)pfile->Get("shapes_fit_b/ch7/Dibosons");
  ...
  leg->AddEntry(prhist, "Pre-fit tt(m)","L");
  leg->AddEntry(pohist, "Post-fit tt(m)","L");
