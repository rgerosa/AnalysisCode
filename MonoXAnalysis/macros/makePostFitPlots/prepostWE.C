...
void prepostWE(string fitFilename, string templateFileName, string observable, Category category, bool isLog = true, bool plotSBFit = false,  bool dumpHisto = false) {
  ...
  wlhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/ZJets_WE").c_str());
  tthist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/Top").c_str());
  dihist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/Dibosons").c_str());
  qcdhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/QCD_WE").c_str());
  if(category == Category::VBF){
    ewkwhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/WJets_EWK").c_str());
    ewkzhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/ZJets_EWK_WE").c_str());
  }
