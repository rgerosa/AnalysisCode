...
void prepostZM2D(string fitFilename, string templateFileName, string observable, int category, bool alongX = false, bool plotSBFit = false) {
  ...
  if(!plotSBFit){
    ...
    dthist = transformUnrolledHistogram((TH1*)dfile->FindObjectAny(("datahistzmm_"+observable).c_str()),observable,category,alongX);
    wlhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch2/WJets_ZM"),observable,category,alongX);
    tthist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch2/Top"),observable,category,alongX);
    dihist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch2/Dibosons"),observable,category,alongX);
    pohist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch2/total_background"),observable,category,alongX);
    prhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_prefit/ch2/total_background"),observable,category,alongX,"prefit");
    
  }
  else{

    dthist = transformUnrolledHistogram((TH1*)dfile->FindObjectAny(("datahistzmm_"+observable).c_str()),observable,category,alongX);
    wlhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch2/WJets_ZM"),observable,category,alongX);
    tthist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch2/Top"),observable,category,alongX);
    dihist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch2/Dibosons"),observable,category,alongX);
    pohist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch2/total_background"),observable,category,alongX);
    prhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_prefit/ch2/total_background"),observable,category,alongX,"prefit");
  ...
  for(size_t ihist = 0 ; ihist < dthist.size(); ihist++){
    ...
    ofstream  outputfile;
    outputfile.open(Form("prepostZM_bin_%d.txt",int(ihist)));
    ...
    leg->AddEntry(pohist.at(ihist), "Post-fit (Z #rightarrow #mu#mu)","L");
    leg->AddEntry(prhist.at(ihist), "Pre-fit (Z #rightarrow #mu#mu)","L");
    ...
    canvas->SaveAs(Form("prepostfit_zmm_bin_%d.pdf",int(ihist)));
    canvas->SaveAs(Form("prepostfit_zmm_bin_%d.png",int(ihist)));
  }
}
