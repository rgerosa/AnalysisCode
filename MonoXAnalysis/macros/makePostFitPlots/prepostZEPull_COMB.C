...
void prepostZEPull_COMB(string fitFilename, string templateFileName, string observable, Category category,bool plotSBFit = false, bool dumpInfo = false) {
  ...
  if(category == Category::monojet)
    frame->GetYaxis()->SetRangeUser(0.001,wlhist->GetMaximum()*200);
  else
  ...
  if(category == Category::monojet)
    frame2->GetYaxis()->SetRangeUser(0.5,1.4);
  else
    frame2->GetYaxis()->SetRangeUser(0.5,1.4);
