...
void makePostFitSig(string   fitFilename, 
		    string   observable, 
		    Category category, 
  ...
  if(!isHiggsInvisible){
    mjhist = (TH1*) pfile->Get(("shapes_prefit/"+dir+"/MonoJ").c_str());
    mwhist = (TH1*) pfile->Get(("shapes_prefit/"+dir+"/MonoW").c_str());
