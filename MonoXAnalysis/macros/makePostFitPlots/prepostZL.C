...
void prepostZL(string fitFilename, string templateFileName, string observable, int category,bool plotSBFit = false) {
  ...
  if(!plotSBFit){
    ...
    dthist = (TH1*)dfile->FindObjectAny(("datahistzll_"+observable).c_str());
    wlhist = (TH1*)pfile->Get("shapes_fit_b/ch2/WJets_ZL");
    ...
    dthist = (TH1*)dfile->FindObjectAny(("datahistzll_"+observable).c_str());
    wlhist = (TH1*)pfile->Get("shapes_fit_s/ch2/WJets_ZL");
  ...
  canvas->SaveAs("prepostfit_zll.pdf");
  canvas->SaveAs("prepostfit_zll.png");
