...
void prepostZM(string fitFilename, string templateFileName, string observable, Category category,bool isLog = true, bool plotSBFit = false,  bool dumpHisto = false) {
  ...
  wlhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/WJets_ZM").c_str());
  tthist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/Top").c_str());
  dihist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/Dibosons").c_str());
  if(category == Category::VBF){
    ewkwhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/WJets_EWK_ZM").c_str());
  ...
  leg->AddEntry(pohist, "Post-fit double-muon CR","L");
  leg->AddEntry(prhist, "Pre-fit double-muon CR","L");
  if(category == Category::VBF)
  ...
  if(dumpHisto){
    ...
    TFile* outFile = new TFile("postfit_weights_ZM.root","RECREATE");
    outFile->cd();
