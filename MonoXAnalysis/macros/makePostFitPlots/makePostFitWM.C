...
void makePostFitWM(string fitFilename, string observable, Category category, bool isCombinedFit = false, bool plotSBFit = false, bool addPullPlot = false,  bool dumpHisto = false) {
  ...
  wlhist = (TH1*)pfile->Get((fit_dir+"/"+dir+"/ZJets_WM").c_str());
  ...
  qcdhist = (TH1*)pfile->Get((fit_dir+"/"+dir+"/FakeM").c_str());
  ...
  if(not addPullPlot){
    ...
    canvas->SaveAs(("prepostfit_wmn"+postfix+"_pull.pdf").c_str());
    canvas->SaveAs(("prepostfit_wmn"+postfix+"_pull.png").c_str());
  }
