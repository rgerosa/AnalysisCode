...
void prepostZE(string fitFilename, string templateFileName, string observable, Category category, bool isLog = true,  bool plotSBFit = false,  bool dumpHisto = false) {
  ...
  wlhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/WJets_ZE").c_str());
  tthist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/Top").c_str());
  dihist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/Dibosons").c_str());
  if(category == Category::VBF){
    ewkwhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/WJets_EWK_ZE").c_str());
    ewkzhist = (TH1*)pfile->Get((dir_fit+"/"+dir+"/Znunu_EWK").c_str());
  }
  ...
  if(isLog){
    if(category == Category::monojet)
      ...
      if(TString(observable).Contains("detajj"))
        ...
        frame->GetYaxis()->SetRangeUser(0.001,wlhist->GetMaximum()*300);
    }
  }
  else
  ...
  leg->AddEntry(pohist, "Post-fit double-electron CR","L");
  leg->AddEntry(prhist, "Pre-fit double-electron CR","L");
  if(category == Category::VBF)
  ...
  if(dumpHisto){
    ...
    TH1* wlhist_prefit = (TH1*) pfile->Get("shapes_prefit/ch2/WJets_ZM");
