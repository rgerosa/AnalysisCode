...
void prepostZE(string fitFilename, string templateFileName, string observable) {
  ...
  TH1* dthist = (TH1*)dfile->Get(("datahistzee_"+observable).c_str());
  TH1* wlhist = (TH1*)pfile->Get("shapes_fit_b/ch5/WJets_ZE");
  TH1* tthist = (TH1*)pfile->Get("shapes_fit_b/ch5/Top_ZE");
  TH1* dihist = (TH1*)pfile->Get("shapes_fit_b/ch5/Dibosons");
  TH1* pohist = (TH1*)pfile->Get("shapes_fit_b/ch5/total_background");    
  TH1* prhist = (TH1*)pfile->Get("shapes_prefit/ch5/total_background");    
  ...
  TH1* frame = canvas->DrawFrame(200., 0.0005, 1000., 1000.0, "");
  ...
  leg->AddEntry(prhist, "Pre-fit Z(ee)","L");
  leg->AddEntry(pohist, "Post-fit Z(ee)","L");
  ...
  d1hist->GetXaxis()->SetLabelSize(3.0*d1hist->GetXaxis()->GetLabelSize());
  d1hist->GetYaxis()->SetLabelSize(3.0*d1hist->GetYaxis()->GetLabelSize());
  d1hist->GetYaxis()->SetRangeUser(0.5,1.5);
  ...
  d1hist->GetYaxis()->SetTitleOffset(0.30);
  d1hist->GetYaxis()->SetLabelSize(0.1);
  ...
  TFile* pcfile = new TFile("postcorrzee.root", "RECREATE");
  postcorr->Write();
  pcfile->Close();
}
