...
void prepostWE2D(string fitFilename, string templateFileName, string observable, int category, bool alongX = false, bool plotSBFit = false) {
  ...
  if(!plotSBFit){
    ...
    dthist = transformUnrolledHistogram((TH1*)dfile->FindObjectAny(("datahistwen_"+observable).c_str()),observable,category,alongX);
    zlhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch6/ZJets_WE"),observable,category,alongX);
    tthist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch6/Top"),observable,category,alongX);
    dihist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch6/Dibosons"),observable,category,alongX);
    pohist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch6/total_background"),observable,category,alongX);
    prhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_prefit/ch6/total_background"),observable,category,alongX,"prefit");
    
  }
  else{

    dthist = transformUnrolledHistogram((TH1*)dfile->FindObjectAny(("datahistwen_"+observable).c_str()),observable,category,alongX);
    zlhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch6/WJets_WE"),observable,category,alongX);
    tthist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch6/Top"),observable,category,alongX);
    dihist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch6/Dibosons"),observable,category,alongX);
    pohist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch6/total_background"),observable,category,alongX);
    prhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_prefit/ch6/total_background"),observable,category,alongX,"prefit");
  ...
  for(size_t ihist = 0 ; ihist < dthist.size(); ihist++){
    ...
    ofstream  outputfile;
    outputfile.open(Form("prepostWE_bin_%d.txt",int(ihist)));
    ...
    leg->AddEntry(pohist.at(ihist), "Post-fit (W #rightarrow e#nu)","L");
    leg->AddEntry(prhist.at(ihist), "Pre-fit (W #rightarrow e#nu)","L");
    ...
    canvas->SaveAs(Form("prepostfit_wen_bin_%d.pdf",int(ihist)));
    canvas->SaveAs(Form("prepostfit_wen_bin_%d.png",int(ihist)));
  }
}
