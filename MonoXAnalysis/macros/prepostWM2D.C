...
void prepostWM2D(string fitFilename, string templateFileName, string observable, int category, bool alongX = false, bool plotSBFit = false) {
  ...
  if(!plotSBFit){
    ...
    dthist = transformUnrolledHistogram((TH1*)dfile->FindObjectAny(("datahistwmn_"+observable).c_str()),observable,category,alongX);
    zlhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch3/ZJets_WM"),observable,category,alongX);
    tthist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch3/Top"),observable,category,alongX);
    dihist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch3/Dibosons"),observable,category,alongX);
    pohist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_b/ch3/total_background"),observable,category,alongX);
    prhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_prefit/ch3/total_background"),observable,category,alongX,"prefit");
    
  }
  else{

    dthist = transformUnrolledHistogram((TH1*)dfile->FindObjectAny(("datahistwmn_"+observable).c_str()),observable,category,alongX);
    zlhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch3/WJets_WM"),observable,category,alongX);
    tthist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch3/Top"),observable,category,alongX);
    dihist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch3/Dibosons"),observable,category,alongX);
    pohist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_fit_s/ch3/total_background"),observable,category,alongX);
    prhist = transformUnrolledHistogram((TH1*)pfile->Get("shapes_prefit/ch3/total_background"),observable,category,alongX,"prefit");
  ...
  for(size_t ihist = 0 ; ihist < dthist.size(); ihist++){
    ...
    ofstream  outputfile;
    outputfile.open(Form("prepostWM_bin_%d.txt",int(ihist)));
    ...
    for(int iBin = 0; iBin < zlhist.at(ihist)->GetNbinsX(); iBin++){
      ZJetRate << "   ";
      ZJetRate << zlhist.at(ihist)->GetBinContent(iBin);
    }
    ...
    zlhist.at(ihist)->SetFillColor(kOrange+1);
    zlhist.at(ihist)->SetLineColor(kBlack);
    zlhist.at(ihist)->Add(tthist.at(ihist));
    zlhist.at(ihist)->Add(dihist.at(ihist));
    
    canvas->cd();
    TH1* frame = (TH1*) dthist.at(ihist)->Clone("frame");
    frame->Reset();
    if(category <=1)
      frame->GetYaxis()->SetRangeUser(0.002,zlhist.at(ihist)->GetMaximum()*50);
    else
      frame->GetYaxis()->SetRangeUser(0.002,zlhist.at(ihist)->GetMaximum()*100);
    ...
    zlhist.at(ihist)->Draw("HIST SAME");
    ...
    leg->AddEntry(pohist.at(ihist), "Post-fit (W #rightarrow #mu#nu)","L");
    leg->AddEntry(prhist.at(ihist), "Pre-fit (W #rightarrow #mu#nu)","L");
    leg->AddEntry(zlhist.at(ihist), "Other Backgrounds", "F");
    leg->Draw("SAME");
    ...
    canvas->SaveAs(Form("prepostfit_wmn_bin_%d.pdf",int(ihist)));
    canvas->SaveAs(Form("prepostfit_wmn_bin_%d.png",int(ihist)));
  }
}
