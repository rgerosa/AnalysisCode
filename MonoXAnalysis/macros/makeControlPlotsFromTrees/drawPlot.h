...
void drawPlot(vector<TH1*> hists, 
	      double xmin, double ymin, 
	      double xmax, double ymax, 
	      const char* dirname = "ouputPlots",
	      const char* cname   = "c", 
	      const char* xlabel  = "", 
	      const char* ylabel = "", 
	      double lumi = 1, 
	      bool drawerrband = false,
	      bool isLog = true) {
  ...
  canvas->cd();
  TPad *pad2 = new TPad(spad2.c_str(), spad2.c_str(), 0, 0.1, 1, 0.299);
  ...
  CMS_lumi(pad1,Form("%.2f",lumi),true);
  ...
  dahist->GetYaxis()->SetRangeUser(0.25,1.75);
  ...
  system(("mkdir -p "+string(dirname)).c_str());
  canvas->Print((string(dirname)+"/"+string(cname)+".pdf").c_str());
  canvas->Print((string(dirname)+"/"+string(cname)+".png").c_str());  
}
