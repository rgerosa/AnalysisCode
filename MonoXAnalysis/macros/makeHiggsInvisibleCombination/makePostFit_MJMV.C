...
void makePostFit_MJMV(string   fitFilename, 
		      Category category, 
  ...
  if(wHhist){
    ...
    if(zHhist) wHhist->Add(zHhist);
    if(ggZHhist) wHhist->Add(ggZHhist);
  }
  ...
  if(category == Category::monojet)
    frame->GetYaxis()->SetRangeUser(0.002,wlhist->GetMaximum()*1000);
  else if(category == Category::monoV)
    frame->GetYaxis()->SetRangeUser(0.01,wlhist->GetMaximum()*1000);
  ...
  if(category == Category::monojet)
    categoryLabel ->DrawLatex(0.175,0.81,"ggH-tagged");
  else if(category == Category::monoV)
    categoryLabel ->DrawLatex(0.175,0.81,"VH-tagged");
  ...
  TLegend* leg = new TLegend(0.40, 0.62, 0.95, 0.92);
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
  leg->SetNColumns(2);
  ...
  if(sighist && plotSBFit)
    ...
    leg->AddEntry(wHhist,  "VH(125) #rightarrow inv.","L");
  }
  ...
  frame3->GetXaxis()->SetTitle("p_{T}^{miss} [GeV]");
