...
void sigdatamchist(TFile* outfile,
  ...
  for(auto obs : observables_2D){
    ...
    TH2F* ewkwhist_temp = new TH2F(("ewkbkgwhist_"+obs+"_2D").c_str(),"",int(bins.binX.size()-1),&bins.binX[0],int(bins.binY.size()-1),&bins.binY[0]);
    TH2F* ewkzhist_temp = new TH2F(("ewkbkgzhist_"+obs+"_2D").c_str(),"",int(bins.binX.size()-1),&bins.binX[0],int(bins.binY.size()-1),&bins.binY[0]);
...
void lepdatamchist(TFile* outfile,
  ...
  if(category != Category::VBF and category != Category::VBFrelaxed){
	...
	vlhists.push_back((TH1*) kfactwjet_vbf->Get("kfactors_shape/kfactor_vbf_mjj_1500_5000_smoothed"));
      ...
      if(not nloSamples.useWJetsNLO){
	vlhists.push_back((TH1*) kfactwjet_vbf->Get("kfactors_cc/kfactor_vbf"));
      }
    }
