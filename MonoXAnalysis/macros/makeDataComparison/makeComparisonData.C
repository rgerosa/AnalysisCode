...
public:
  eventID(unsigned int & eventid, unsigned int & lumiid, unsigned int & runid){
    eventid_ = eventid;
    runid_ = runid;
    lumiid_ = lumiid;
  }
 ...
 bool operator== (const eventID & a) const{
    if(eventid_ == a.eventid_ and runid_ == a.runid_ and lumiid_ == a.lumiid_) return true;
    else return false;
  }
 ...
 bool operator< (const eventID & a) const{
    if(runid_ < a.runid_) return true;
      ...
      if(lumiid_ < a.lumiid_) return true;
      else if(lumiid_ > a.lumiid_) return false;
      else {
...
void makeComparisonData(string inputDIR_1, string inputDIR_2, Sample sample, Category category, string outputDIR, bool useOnlyICHEP){
  ...
  unsigned int njetcentral = 0, njetinclusive = 0, runnumber = 0, eventnumber = 0, luminumber = 0;
  ...
  tree_1->Branch("luminumber",&luminumber,"luminumber/i");
  ...
  while(reader_1.Next()){
    ...
    njetcentral = 0; njetinclusive = 0; runnumber = 0; eventnumber = 0; luminumber = 0;
    ...
    eventPassing_chain1.push_back(eventID(*event,*lumi,*run));
    ...
    if (sample == Sample::sig || sample == Sample::qcd){ 
      ...
      leadingjetpt = jetpt->at(0); leadingjeteta =  jeteta->at(0); njetcentral = *njets; njetinclusive = *nincjets; runnumber = *run; eventnumber = *event; luminumber = *lumi;
      if(jetpt->size() > 1){
  ...
  tree_2->Branch("luminumber",&luminumber,"luminumber/i");
  ...
  while(reader_2.Next()){
    ...
    eventPassing_chain2.push_back(eventID(*event_alt,*lumi_alt,*run_alt));
    ...
    if (sample == Sample::sig || sample == Sample::qcd){ 
      ...
      leadingjetpt = jetpt_alt->at(0); leadingjeteta =  jeteta_alt->at(0); njetcentral = *njets_alt; njetinclusive = *nincjets_alt; 
      runnumber = *run_alt; eventnumber = *event_alt; luminumber = *lumi_alt; luminumber = *lumi;
	...
	secondjeteta = jeteta_alt->at(1);
      ...
      leadingjetpt = jetpt_alt->at(0); leadingjeteta =  jeteta_alt->at(0); njetcentral = *njets_alt; njetinclusive = *nincjets_alt; runnumber = *run_alt; eventnumber = *event_alt;  luminumber = *lumi;
      if(jetpt_alt->size() > 1){
	secondjetpt = jetpt_alt->at(1);
	secondjeteta = jeteta_alt->at(1);
      }      
  ...
  TTreeReaderValue<unsigned int> luminumber_1   (myreader1,"luminumber");
  ...
  unsigned int njetcentral_2, njetinclusive_2, runnumber_2, eventnumber_2, luminumber_2;
  ...
  tree_2->SetBranchAddress("luminumber",&luminumber_2);

  while(myreader1.Next()){
    eventID event_tmp(*eventnumber_1,*luminumber_1,*runnumber_1);
	...
	if(event_tmp.lumiid_ != luminumber_2) continue;
	// when found
  ...
  for(long int iEvent = 0; iEvent < tree_2->GetEntries(); iEvent++){
    tree_2->GetEntry(iEvent);
    eventID event_tmp(eventnumber_2,luminumber_2,runnumber_2);
  ...
  ofstream file_nonIn2 ("events_notIn2.txt");
  ...
  while(reader_2.Next()){
    ...
    eventID event_tmp(*event_alt,*lumi_alt,*run_alt);
    auto iterator = std::find(eventIn1Only.begin(),eventIn1Only.end(),event_tmp);
    if(iterator != eventIn1Only.end()){

      eventIn1Only.erase(iterator);
      ...
      if(*nbjets_alt != 0){
	file_nonIn2 << *run_alt << ":"<<*lumi_alt<<":"<<*event_alt<<" \n";
	continue;
      }
  ...
  for(auto eventid : eventIn1Only)
    file_nonIn2 << eventid.runid_<< ":"<<eventid.lumiid_<<":"<<eventid.eventid_<<" \n";
  ...
  ofstream file_nonIn1 ("events_notIn1.txt");
  ...
  for(auto eventid : eventIn2Only)
      file_nonIn1 << eventid.runid_ <<":"<<eventid.lumiid_<<":"<<eventid.eventid_<<" \n";
