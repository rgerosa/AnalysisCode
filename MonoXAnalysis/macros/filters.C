...
vector<string> fileListForChain (const std::string path, bool isEOS){
  ...
  if(isEOS)
    system(("eos find "+path+" -name \"*.root\" > file.temp").c_str());
  else
  ...
  if(infile.is_open()){
    while(!infile.eof()){
      ...
      if(line == "") continue;
      if(isEOS)
	outputFileList.push_back("root://eoscms.cern.ch//eos/cms"+line);
      else
...
void gamfilter(std::string inputFileName,  // name of a single file or directory path                                                                                           
  ...
  // medium id + pt + veto
  const char* cut = "nmuons == 0 && nelectrons == 0 && ntaus == 0 && nphotons == 1 && phpt > 120 && phidm == 1";
...
void topmufilter(std::string inputFileName,  // name of a single file or directory path                                                                                        
  ...
  const char* cut = "nmuons == 1 && nelectrons == 0 && ntaus == 0 && nphotons == 0 && nbjets > 0 && mu1id >=1 && mu1pt > 20";
...
void topelfilter(std::string inputFileName,  // name of a single file or directory path                                                                                        
  ...
  // one tight muon + b-jet --> semi-leptonic ttbar events
  const char* cut = "nmuons == 0 && nelectrons == 1 && ntaus == 0 && nphotons == 0 && nbjets > 0 && el1id >=1 && el1pt > 40";
