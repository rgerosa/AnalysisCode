...
void plotAxial(string inputDIR, string outputDIR, string coupling = "025", string energy = "13") {
  ...
  TH1* frame = canvas->DrawFrame(150., 0., 1800., 1000.0, "");
  ...
  if (coupling == "1"){
    tex->DrawLatex(0.36,0.90,"#bf{Axial-vector, Dirac, g_{q} = 1, g_{DM} = 1}");
  }
  else
    tex->DrawLatex(0.36,0.90,"#bf{Axial-vector, Dirac, g_{q} = 0.25, g_{DM} = 1}");
