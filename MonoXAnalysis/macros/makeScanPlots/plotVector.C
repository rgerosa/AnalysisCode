...
void plotVector(string inputDIR, string outputDIR, string coupling = "025", string energy = "13") {
  ...
  TFile *file = new TFile((inputDIR+"/limits_800_"+string(coupling)+"_v1_"+string(energy)+"TeV.root").c_str());  
  TTree *tree = (TTree*)file->Get("limit");
  // additional lines
  TFile* file2 = new TFile((inputDIR+"/vector_out.root").c_str());
  TGraph* wm = (TGraph*)file2->Get("wmap_0");
  TGraph* dd = (TGraph*)file2->Get("DD_mass");
  TGraph* hexp8 = (TGraph*)file2->Get("combined_expected");
  TGraph* hobs8 = (TGraph*)file2->Get("combined");
  ...
  TH1* frame = canvas->DrawFrame(150., 0., 2500., 1000.0, "");
  ...
  hexp8->SetLineColor(kRed);
  hexp8->SetLineWidth(3);
  hexp8->SetLineStyle(2);
  hobs8->SetLineColor(kRed);
  hobs8->SetLineWidth(3);
  
  hexp->SetMinimum(0.01);
  hexp->SetMaximum(10.);
  hobs->SetMinimum(0.01);
  hobs->SetMaximum(10.);
  hobu->SetMinimum(0.01);
  hobu->SetMaximum(10.);
  hobd->SetMinimum(0.01);
  hobd->SetMaximum(10.);
  
  hobs2->SetLineColor(2);
  hexp2->SetLineColor(kBlue);
  hobs8->SetLineColor(1);
  hexp8->SetLineColor(1);
  ...
  TLegend *leg = new TLegend(0.22,0.62,0.68,0.88,NULL,"brNDC");
  leg->AddEntry(hexp2,"Median Expected (13 TeV) 95% CL","L");
  leg->AddEntry(hexp2_up,"Expected (13 TeV) #pm 1#sigma_{experiment} ","L");
  ...
  tex->DrawLatex(0.24,0.90,"#bf{Vector, Dirac, g_{q} = "+TString(coupling)+", g_{DM} = 1}");
