...
void plotPseudoScalar(string inputDIR, string outputDIR, string coupling = "025", string energy = "13") {
  ...
  TFile *file = new TFile((inputDIR+"/limits_806_"+string(coupling)+"_v1_"+string(energy)+"TeV.root").c_str());  
  TTree *tree = (TTree*)file->Get("limit");

  TFile* file2 = new TFile((inputDIR+"/pseudoscalar_out.root").c_str());
  ...
  for (int i = 1; i <= hexp2->GetNbinsX(); i++) {
    ...
    TH1* frame = canvas->DrawFrame(0., 0., 1000., 600.0, "");
    ...
    if (coupling == "1"){
        tex->DrawLatex(0.36,0.90,"#bf{Pseudo Scalar-vector, Dirac, g_{q} = 1, g_{DM} = 1}");
    }
    else
        tex->DrawLatex(0.36,0.90,"#bf{Pseudo Scalar-vector, Dirac, g_{q} = 0.25, g_{DM} = 1}");
