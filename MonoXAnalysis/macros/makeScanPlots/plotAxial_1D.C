...
void plotAxial_1D(string inputFileName, string outputDIR, int dmMass = 1, string coupling = "025") {
  ...
  tex->Draw();
  if (coupling == "1")
    tex->DrawLatex(0.175,0.80,("#bf{Axial-vector med, Dirac DM, m_{med} = "+to_string(dmMass)+" GeV g_{q} = 1, g_{DM} = 1}").c_str());
  else
    tex->DrawLatex(0.175,0.80,("#bf{Axial-vector med, Dirac DM, m_{med} = "+to_string(dmMass)+" GeV g_{q} = 0.25, g_{DM} = 1}").c_str());
  
  gPad->RedrawAxis();
  gPad->Modified(); 
  gPad->Update();
  
  canvas->SaveAs((outputDIR+"/scan_axial_1D_dmMass_"+to_string(dmMass)+"_g"+string(coupling)+".pdf").c_str(),"pdf");
