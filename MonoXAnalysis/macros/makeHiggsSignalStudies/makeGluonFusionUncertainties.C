...
void makeGluonFusionUncertainties(float njet, string outputName){
  ...
  TGraph* migration01 = new TGraph() ;
  TGraph* migration12 = new TGraph() ;
  TGraph* higgsPt   = new TGraph() ;
  TGraph* topEffect = new TGraph() ;
  ...
  scaleVariation->Write("scaleVariation");
  resummation->Write("resummation");
  migration01->Write("migration01");
  migration12->Write("migration12");
  higgsPt->Write("higgsPt");
  topEffect->Write("topEffect");
  outputFile->Close();
}
