...
static bool  correlateEWK  = false;  // to correlate EWK uncertainties across bins on the Z/gamma Z/W ratio
...
static bool  runOnlyBackground = false;    // run only on signal templates --> workspace with only background
