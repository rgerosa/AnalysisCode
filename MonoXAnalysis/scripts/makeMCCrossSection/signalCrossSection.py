...
def computeXS(med,dm,gq,proc):
    ...
    lWHist = r.TH1F("W","W",1,-100000,10000000)
    lXHist = r.TH1F("X","X",1,-100000,10000000)
    lTree.Draw("v_pt>>W","evtweight*(genmet > 0)")
    lTree.Draw("v_pt>>X","evtweight*(genmet > 200)")
    #print sumxs,sumweights,sumentries
    sumweights += lWHist.Integral()
    sumxs      += lXHist.Integral()
    sumentries += lTree.GetEntries()
...
def computeXSV(med,dm,gq,proc,iId):
    ...
    if float(gq) == 1:
        ...
        gq = '0.25'
    if int(iId) == 23:
        infile='%s/MonoZ_%s_%s_%s_%s_%s.root' % (basedir,med,dm,gdm,gq,proc)
    elif int(iId) == 24:
        infile='%s/MonoW_%s_%s_%s_%s_%s.root' % (basedir,med,dm,gdm,gq,proc)
    ...
    weight1+="*"
    print weight1
    lTree.Draw("v_pt>>W",str(weight1)+"(v_pt > 0)*"+str(bosonid))
    lTree.Draw("v_pt>>X",str(weight1)+"(v_pt > 200)*"+str(bosonid))
